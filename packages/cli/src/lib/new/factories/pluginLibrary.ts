...
export const pluginLibrary = createFactory<Options>({
  name: 'plugin',
  description: 'A new plugin-library',
  ...
  async create(options: Options, ctx: CreateContext) {
    ...
    Task.log();
    Task.log(`Creating plugin ${chalk.cyan(name)}`);
    ...
    await executePluginPackageTemplate(ctx, {
      targetDir,
      templateName: 'default-plugin-library',
      values: {
        id,
        name,
        extensionName,
